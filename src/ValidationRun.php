...
final class ValidationRun
{
	...
	private function finalizeItem(array $values): array
	{
		...
		foreach ($this->shape->fields as $field => $definition) {
			...
			$values[$field] = new \Celema\Sire\Value($finalValue, $value->pristine, $value->empty);
		}
	...
	private function readExtraValue(string $field, mixed $value, string|int|null $listIndex): ?Value
	{
		if ($this->shape->extra === Extra::Allow) {
			return new \Celema\Sire\Value($value, $value, self::isRawEmptyValue($value));
		}
	...
	private function readKnownValue(Field $definition, mixed $value): ReadValue
	{
		if ($value === null) {
			return new ReadValue(
				new \Celema\Sire\Value(null, null, true),
		...
		return new ReadValue(
			new \Celema\Sire\Value($coercion->value, $coercion->pristine, $coercion->empty),
	...
	private function toSubValues(
		...
		if (!is_array($pristine)) {
			return new ReadValue(
				new \Celema\Sire\Value($pristine, $pristine, self::isRawEmptyValue($pristine)),
		...
		if ($result->valid()) {
			...
			return new ReadValue(new \Celema\Sire\Value($values, $pristine, $values === []));
		}
